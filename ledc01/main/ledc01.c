...
void config_timers() {
	...
	printf("low speed timer config failed\n");
	...
	printf("high speed timer config failed\n");
	vTaskDelay(DELAY_ERROR / portTICK_PERIOD_MS);
    }
}
...
void app_main(void) {
    ...
    for (ch = 0; ch < LEDC_TEST_CH_NUM; ch++) {
	    ...
	    printf("ledc_channel_config failed, ch: %d\n", ch);
	    vTaskDelay(DELAY_ERROR / portTICK_PERIOD_MS);
	}
    }
    ...
    for (;;) {
		...
		printf("ledc_channel_config failed 1, ch: %d\n", ch);
		vTaskDelay(DELAY_ERROR / portTICK_PERIOD_MS);
	    }
		...
		printf("ledc_channel_config failed 2, ch: %d\n", ch);
		vTaskDelay(DELAY_ERROR / portTICK_PERIOD_MS);
	    }
        }
